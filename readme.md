...
So is a projection, an update, an aggregation pipeline, an expression, and a process.
...
- ***Running work which has to survive being interrupted.***
  A process is a JSON document and a run is a JSON value, so a half-finished job can be written
  to storage, moved to another machine, and picked up later.
...
The expression language is the same wherever it turns up: inside an `$expr` in a query, in a
  computed field of a projection, in an `$addFields` stage, and in a `$do` step of a process, it
  is the same `Evaluate()`.
...
The first five families are MongoDB's, and `jsongin` implements 86.2% of them:
  219 of the 254 operators MongoDB documents.
The sixth is jsongin's own, because MongoDB has no process language.
...
- More Than Queries:
	...
	- Define, store, and run a JSON based process, and resume one which was interrupted.

- Developer Features:
	- No external dependencies. None. Zero.
	- 100% pure javascript, on the server and in the browser.
	- Single minified file (~194k, ~42k compressed) for web deployment.
