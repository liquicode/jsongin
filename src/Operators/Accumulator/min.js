...
Values are ordered by `CompareValues()`, which follows MongoDB's BSON type order, so a group
  holding values of different types still has a well defined smallest value.

Note that this is the accumulator `$min`. There is also an expression operator `$min`, which
  selects the smallest of several values within one document, and an update operator `$min`,
...
module.exports = function ( jsongin )
{
	...
	let operator =
	{
		...
		Accumulate: function ( Documents, Args, Scope )
		{
			...
			{
				if ( jsongin.OpError ) { jsongin.OpError( `Accumulator.$min: ${error.message}` ); }
				throw error;
			}
		},
