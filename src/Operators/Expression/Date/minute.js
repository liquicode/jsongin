...
## Operators > Expression > $minute

Usage: `$minute: expression`
  or `$minute: { date: expression, timezone: string }`
...
module.exports = function ( jsongin )
{
	...
	let operator =
	{
		...
		Evaluate: function ( Document, Args, Scope )
		{
			try
			{
				let read = date.ReadDateArgs( Document, Args, '$minute', [], Scope );
				if ( read === null ) { return null; }

				let shifted = date.ZoneShifted( read.Date, read.Zone, '$minute' );
				return ( shifted.getUTCMinutes() );
			}
			catch ( error )
			{
				if ( jsongin.OpError ) { jsongin.OpError( `Expression.$minute: ${error.message}` ); }
				throw error;
			}
		},
